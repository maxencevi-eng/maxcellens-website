...
export default async function RootLayout({ children }: { children: ReactNode }) {
  ...
  // NOTE: defer any DOM class toggles so we don't change server HTML before React hydrates
  const fontLoaderScript = `<script>(function(){try{var timeout=1000;function setLoaded(){try{document.documentElement.classList.remove('wf-loading');document.documentElement.classList.add('wf-loaded');}catch(e){}}function setFailed(){try{document.documentElement.classList.remove('wf-loading');document.documentElement.classList.add('wf-failed');}catch(e){}}if(document.fonts&&document.fonts.ready){document.fonts.ready.then(function(){try{setTimeout(setLoaded,0);}catch(e){}});setTimeout(function(){try{if(document.documentElement.classList.contains('wf-loading')){setTimeout(setFailed,0);}}catch(e){}},timeout);}else{try{setTimeout(setLoaded,0);}catch(e){}}}catch(e){} })()</script>`;
