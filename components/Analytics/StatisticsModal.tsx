...
export default function StatisticsModal({
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      // Sécurisation des données pour éviter les plantages si l'API renvoie des null/undefined

      const safeData: AnalyticsData = {
        kpis: { ...DEFAULT_KPIS, ...(json?.kpis || {}) },
        trafficLast30Days: Array.isArray(json?.trafficLast30Days) ? json.trafficLast30Days : [],
        topContent: Array.isArray(json?.topContent) ? json.topContent : [],
        byCountry: Array.isArray(json?.byCountry) ? json.byCountry : [],
        byCity: Array.isArray(json?.byCity) ? json.byCity : [],
        topClicks: Array.isArray(json?.topClicks) ? json.topClicks : [],
        bySource: Array.isArray(json?.bySource) ? json.bySource : [],
        visitsByPage: Array.isArray(json?.visitsByPage) ? json.visitsByPage : [],
        visitors: Array.isArray(json?.visitors) ? json.visitors : [],
        filterApplied: {
          include: json?.filterApplied?.include ?? false,
          exclude: json?.filterApplied?.exclude ?? false,
        },
  ...
  const addHashToExclude = (ip_hash: string | null) => {
    if (!ip_hash || !ip_hash.trim()) return;
    setIpFilter((prev) => {
  ...
  const addHashesToExclude = (hashes: string[]) => {
    const toAdd = hashes.filter((h) => h && h.trim());
  ...
  if (data && data.kpis) kpis = data.kpis;
  const uniqueVisitorsValue = kpis.uniqueVisitors;
  const totalViewsValue = kpis.totalViews;
  const avgPagesPerVisitValue = kpis.avgPagesPerVisit;
  const avgTimePerPageSecondsValue = kpis.avgTimePerPageSeconds;
  const bounceRatePercentValue = kpis.bounceRatePercent;
  const excludeHashesList = ipFilter.excludeHashes;
  const excludeHashesText = excludeHashesList.join('\n');
          ...
          <div className={styles.contentPadding}>
            ...
            <div className={styles.kpiGrid} style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16, marginBottom: 24 }}>
              <div style={{ background: '#f1f5f9', padding: 16, borderRadius: 10, border: '1px solid #e2e8f0' }}>
                ...
                <div className={styles.kpiValue} style={{ fontSize: 26, fontWeight: 700, color: '#0f172a' }}>{uniqueVisitorsValue}</div>
              </div>
              <div style={{ background: '#f1f5f9', padding: 16, borderRadius: 10, border: '1px solid #e2e8f0' }}>
                <div style={{ fontSize: 13, color: '#475569', marginBottom: 6, fontWeight: 600 }}>Total vues</div>
                <div className={styles.kpiValue} style={{ fontSize: 26, fontWeight: 700, color: '#0f172a' }}>{totalViewsValue}</div>
              </div>
              <div style={{ background: '#f1f5f9', padding: 16, borderRadius: 10, border: '1px solid #e2e8f0' }}>
                <div style={{ fontSize: 13, color: '#475569', marginBottom: 6, fontWeight: 600 }}>Pages / visite</div>
                <div className={styles.kpiValue} style={{ fontSize: 26, fontWeight: 700, color: '#0f172a' }}>{avgPagesPerVisitValue}</div>
              </div>
              <div style={{ background: '#f1f5f9', padding: 16, borderRadius: 10, border: '1px solid #e2e8f0' }}>
                <div style={{ fontSize: 13, color: '#475569', marginBottom: 6, fontWeight: 600 }}>Temps moyen / page</div>
                <div className={styles.kpiValue} style={{ fontSize: 26, fontWeight: 700, color: '#0f172a' }}>{formatDurationSeconds(avgTimePerPageSecondsValue)}</div>
              </div>
              <div style={{ background: '#f1f5f9', padding: 16, borderRadius: 10, border: '1px solid #e2e8f0' }}>
                <div style={{ fontSize: 13, color: '#475569', marginBottom: 6, fontWeight: 600 }}>Taux de rebond</div>
                <div className={styles.kpiValue} style={{ fontSize: 26, fontWeight: 700, color: '#0f172a' }}>{bounceRatePercentValue}%</div>
              </div>
            </div>
            ...
            <p style={{ fontSize: 13, color: '#64748b', marginBottom: 16, padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #e2e8f0' }}>
              <strong>Pourquoi je ne vois pas mes visites ?</strong> Les visites sont enregistrées uniquement lorsque vous n&apos;êtes <strong>pas connecté (admin)</strong>. Fermer l&apos;onglet ne suffit pas : déconnectez-vous (menu Admin → Déconnexion) ou utilisez une <strong>fenêtre de navigation privée</strong> pour tester.
            </p>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              {dataTab === 'content' && (
                (() => {
                  const contentData = Array.isArray(data?.topContent) ? data.topContent : [];
                  const showPagination = contentData.length > PAGE_SIZE;
                  return (
                    <div>
                      ...
                      <div style={{ overflowX: 'auto', border: '1px solid #e2e8f0', borderRadius: 10 }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                          ...
                          <tbody>
                            {(contentData.slice((pageContent - 1) * PAGE_SIZE, pageContent * PAGE_SIZE)).map((row) => (
                      ...
                      {showPagination && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 10, fontSize: 14 }}>
                          ...
                          <span style={{ color: '#475569' }}>Page {pageContent} / {Math.ceil(contentData.length / PAGE_SIZE)}</span>
                          <button type="button" onClick={() => setPageContent((p) => Math.min(Math.ceil(contentData.length / PAGE_SIZE), p + 1))} disabled={pageContent >= Math.ceil(contentData.length / PAGE_SIZE)} style={{ padding: '6px 12px', border: '1px solid #cbd5e1', borderRadius: 6, background: '#fff', cursor: pageContent >= Math.ceil(contentData.length / PAGE_SIZE) ? 'not-allowed' : 'pointer', opacity: pageContent >= Math.ceil(contentData.length / PAGE_SIZE) ? 0.6 : 1 }}>Suivant</button>
                        </div>
                      )}
                    </div>
                  );
              ...
              {dataTab === 'geo' && (
                (() => {
                  const countryData = Array.isArray(data?.byCountry) ? data.byCountry : [];
                  const cityData = Array.isArray(data?.byCity) ? data.byCity : [];
                  const showCountryPagination = countryData.length > PAGE_SIZE;
                  const showCityPagination = cityData.length > PAGE_SIZE;
                  return (
                      ...
                      <div>
                        ...
                        {showCountryPagination && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 10, fontSize: 14 }}>
                            ...
                            <span style={{ color: '#475569' }}>Page {pageGeoCountry} / {Math.ceil(countryData.length / PAGE_SIZE)}</span>
                            <button type="button" onClick={() => setPageGeoCountry((p) => Math.min(Math.ceil(countryData.length / PAGE_SIZE), p + 1))} disabled={pageGeoCountry >= Math.ceil(countryData.length / PAGE_SIZE)} style={{ padding: '6px 12px', border: '1px solid #cbd5e1', borderRadius: 6, background: '#fff', cursor: pageGeoCountry >= Math.ceil(countryData.length / PAGE_SIZE) ? 'not-allowed' : 'pointer', opacity: pageGeoCountry >= Math.ceil(countryData.length / PAGE_SIZE) ? 0.6 : 1 }}>Suivant</button>
                          </div>
                        )}
                      </div>
                      <div>
                        ...
                        <div style={{ overflowX: 'auto', border: '1px solid #e2e8f0', borderRadius: 10 }}>
                          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                            ...
                            <tbody>
                              {(cityData.slice((pageGeoCity - 1) * PAGE_SIZE, pageGeoCity * PAGE_SIZE)).map((row) => (
                        ...
                        {showCityPagination && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 10, fontSize: 14 }}>
                            ...
                            <span style={{ color: '#475569' }}>Page {pageGeoCity} / {Math.ceil(cityData.length / PAGE_SIZE)}</span>
                            <button type="button" onClick={() => setPageGeoCity((p) => Math.min(Math.ceil(cityData.length / PAGE_SIZE), p + 1))} disabled={pageGeoCity >= Math.ceil(cityData.length / PAGE_SIZE)} style={{ padding: '6px 12px', border: '1px solid #cbd5e1', borderRadius: 6, background: '#fff', cursor: pageGeoCity >= Math.ceil(cityData.length / PAGE_SIZE) ? 'not-allowed' : 'pointer', opacity: pageGeoCity >= Math.ceil(cityData.length / PAGE_SIZE) ? 0.6 : 1 }}>Suivant</button>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div style={{ overflowX: 'auto', border: '1px solid #e2e8f0', borderRadius: 10 }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                          ...
                          <tbody>
                            {(clicksData.slice((pageClicks - 1) * PAGE_SIZE, pageClicks * PAGE_SIZE)).map((row) => (
                      ...
                      {showPagination && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 10, fontSize: 14 }}>
                          ...
                          <span style={{ color: '#475569' }}>Page {pageClicks} / {Math.ceil(clicksData.length / PAGE_SIZE)}</span>
                          <button type="button" onClick={() => setPageClicks((p) => Math.min(Math.ceil(clicksData.length / PAGE_SIZE), p + 1))} disabled={pageClicks >= Math.ceil(clicksData.length / PAGE_SIZE)} style={{ padding: '6px 12px', border: '1px solid #cbd5e1', borderRadius: 6, background: '#fff', cursor: pageClicks >= Math.ceil(clicksData.length / PAGE_SIZE) ? 'not-allowed' : 'pointer', opacity: pageClicks >= Math.ceil(clicksData.length / PAGE_SIZE) ? 0.6 : 1 }}>Suivant</button>
                        </div>
                      )}
                    </div>
                  );
              ...
              {dataTab === 'source' && (
                <div>
                  ...
                  {(() => {
                    // Sécurité additionnelle pour éviter les erreurs de variables non définies
                    const sourceData = Array.isArray(data?.bySource) ? data.bySource : [];
                    const showPagination = sourceData.length > PAGE_SIZE;
                    return (
                      <>
                        <div style={{ overflowX: 'auto', border: '1px solid #e2e8f0', borderRadius: 10 }}>
                          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                            ...
                            <tbody>
                              {(sourceData.slice((pageSource - 1) * PAGE_SIZE, pageSource * PAGE_SIZE)).map((row, i) => (
                        ...
                        {showPagination && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 10, fontSize: 14 }}>
                            ...
                            <span style={{ color: '#475569' }}>Page {pageSource} / {Math.ceil(sourceData.length / PAGE_SIZE)}</span>
                            <button type="button" onClick={() => setPageSource((p) => Math.min(Math.ceil(sourceData.length / PAGE_SIZE), p + 1))} disabled={pageSource >= Math.ceil(sourceData.length / PAGE_SIZE)} style={{ padding: '6px 12px', border: '1px solid #cbd5e1', borderRadius: 6, background: '#fff', cursor: pageSource >= Math.ceil(sourceData.length / PAGE_SIZE) ? 'not-allowed' : 'pointer', opacity: pageSource >= Math.ceil(sourceData.length / PAGE_SIZE) ? 0.6 : 1 }}>Suivant</button>
                          </div>
                        )}
                      </>
                    );
              ...
              {dataTab === 'visitsByPage' && (
                <div>
                  ...
                  {(() => {
                    // Sécurité additionnelle pour éviter les erreurs de variables non définies
                    const visitsData = Array.isArray(data?.visitsByPage) ? data.visitsByPage : [];
                    const showPagination = visitsData.length > PAGE_SIZE;
                    return (
                      <>
                        <div style={{ overflowX: 'auto', border: '1px solid #e2e8f0', borderRadius: 10 }}>
                          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                            ...
                            <tbody>
                              {(visitsData.slice((pageVisitsByPage - 1) * PAGE_SIZE, pageVisitsByPage * PAGE_SIZE)).map((row) => (
                                <tr key={row.path} style={{ borderBottom: '1px solid #e2e8f0' }}>
                                  <td style={{ padding: '10px 14px', color: '#1e293b' }}>Page {getPageLabel(row.path || '/')}</td>
                        ...
                        {showPagination && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 10, fontSize: 14 }}>
                            ...
                            <span style={{ color: '#475569' }}>Page {pageVisitsByPage} / {Math.ceil(visitsData.length / PAGE_SIZE)}</span>
                            <button type="button" onClick={() => setPageVisitsByPage((p) => Math.min(Math.ceil(visitsData.length / PAGE_SIZE), p + 1))} disabled={pageVisitsByPage >= Math.ceil(visitsData.length / PAGE_SIZE)} style={{ padding: '6px 12px', border: '1px solid #cbd5e1', borderRadius: 6, background: '#fff', cursor: pageVisitsByPage >= Math.ceil(visitsData.length / PAGE_SIZE) ? 'not-allowed' : 'pointer', opacity: pageVisitsByPage >= Math.ceil(visitsData.length / PAGE_SIZE) ? 0.6 : 1 }}>Suivant</button>
                          </div>
                        )}
                      </>
                    );
              ...
              {dataTab === 'visitors' && (
                <VisitorsTab
                  ...
                  setPageVisitors={setPageVisitors}
                  selectedVisitorHashes={selectedVisitorHashes}
                  setSelectedVisitorHashes={setSelectedVisitorHashes}
                  addIpToExclude={addIpToExclude}
                  addHashToExclude={addHashToExclude}
                  hashAddedFeedback={hashAddedFeedback}
                  formatCity={formatCity}
                  addHashesToExclude={addHashesToExclude}
                />
              )}
            </div>
              ...
              <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: 16, marginTop: 8, marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
                  <button
                    ...
                    disabled={excludeHashesCount === 0}
                    style={{ padding: '6px 12px', fontSize: 12, background: excludeHashesCount ? '#fef3c7' : '#f1f5f9', color: excludeHashesCount ? '#92400e' : '#94a3b8', border: '1px solid #fcd34d', borderRadius: 6, cursor: excludeHashesCount ? 'pointer' : 'not-allowed' }}
                  >
                    Copier les hash
                  </button>
                  <button
                    ...
                    disabled={purgingHashes || excludeHashesCount === 0}
                    style={{ padding: '6px 12px', fontSize: 12, background: purgingHashes || excludeHashesCount === 0 ? '#f1f5f9' : '#dc2626', color: purgingHashes || excludeHashesCount === 0 ? '#94a3b8' : '#fff', border: 'none', borderRadius: 6, cursor: purgingHashes || excludeHashesCount === 0 ? 'not-allowed' : 'pointer', fontWeight: 600 }}
                  >
                ...
                {excludeHashesCount > 0 && (
                  <p style={{ fontSize: 11, color: '#64748b', marginTop: 6 }}>
                    {excludeHashesCount} hash. Cliquez sur &quot;Enregistrer le filtre&quot; ci-dessous pour persister l&apos;exclusion dans les stats.
                  </p>
                )}
              </div>
