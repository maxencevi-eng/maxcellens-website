...
// AdminRevelations.tsx — Gestion des Dénouements (scènes de finale scripted, formerly Révélations)
import { useEffect, useState } from 'react';
import type { BacDenouement, BacProfil, BacRole, ScriptBloc, ItwQuestion, NotesRealisation } from '../../lib/bac/types';

const EMPTY_NOTES: NotesRealisation = { cadrage: '', rythme: '', silences: '', pieges: '', astuce: '' };

// ─── Éditeur complet ───────────────────────────────────────────────────────────
function DenouementEditor({
  item,
  roles,
  groupes,
  onClose,
  ...
  async function save() {
    const res = await fetch('/bac/api/denouements', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: item.id, ...form }),
    });
    if (res.ok) { onToast('Dénouement enregistré'); }
    else { onToast('Erreur', 'error'); }
  }

  async function handleDelete() {
    if (!confirm(`Supprimer "${item.titre}" définitivement ?`)) return;
    const res = await fetch(`/bac/api/denouements?id=${item.id}`, { method: 'DELETE' });
    if (res.ok) { onToast('Dénouement supprimé'); onClose(); }
    else { onToast('Erreur', 'error'); }
  }
  ...
  function removeBloc(i: number) { setField('script_json', form.script_json.filter((_, idx) => idx !== i)); }
  ...
  function updateBloc(i: number, patch: Partial<ScriptBloc>) {
    setField('script_json', form.script_json.map((b, idx) => idx === i ? { ...b, ...patch } as ScriptBloc : b));
  }
    ...
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, gap: 12, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span className={`bac-badge ${form.actif ? 'bac-badge-success' : 'bac-badge-error'}`}>{form.actif ? 'Actif' : 'Inactif'}</span>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="bac-btn bac-btn-ghost bac-btn-sm" onClick={() => setField('actif', !form.actif)}>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 24, borderBottom: '2px solid var(--bac-border)', paddingBottom: 0 }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id as any)} className="bac-btn bac-btn-ghost bac-btn-sm"
            style={{
              borderRadius: '8px 8px 0 0', borderBottom: tab === t.id ? '2px solid var(--bac-primary)' : '2px solid transparent',
              color: tab === t.id ? 'var(--bac-primary)' : 'var(--bac-text-secondary)', fontWeight: tab === t.id ? 700 : 400, marginBottom: -2,
            }}
          >{t.label}</button>
        ))}
      </div>
        ...
        <div className="bac-card" style={{ padding: 24 }}>
          ...
          <div className="bac-form-row">
            <div className="bac-form-group">
              ...
              <input className="bac-input" value={form.ton_principal} onChange={e => setField('ton_principal', e.target.value)} placeholder="ex: Festif" />
            </div>
            <div className="bac-form-group">
              <label className="bac-label">Ton secondaire</label>
              <input className="bac-input" value={form.ton_secondaire} onChange={e => setField('ton_secondaire', e.target.value)} placeholder="ex: Émouvant" />
            </div>
          </div>
          <div className="bac-form-row">
            <div className="bac-form-group">
          ...
          <div className="bac-form-group">
            <label className="bac-label">Groupes avec des rôles dans ce dénouement</label>
      ...
      {tab === 'script' && (
        <div>
          ...
          {form.script_json.length === 0 && <div className="bac-empty"><p>Aucun bloc de script</p></div>}
              ...
              <div key={i} className="bac-card" style={{ padding: 16, borderLeft: `4px solid ${bloc.type === 'didascalie' ? '#f59e0b' : 'var(--bac-primary)'}` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  ...
                  <div style={{ display: 'flex', gap: 4 }}>
                    <button className="bac-btn bac-btn-ghost bac-btn-icon" onClick={() => moveBloc(i, -1)}>↑</button>
                    <button className="bac-btn bac-btn-ghost bac-btn-icon" onClick={() => moveBloc(i, 1)}>↓</button>
                ...
                {bloc.type === 'didascalie' ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    <textarea className="bac-input bac-textarea" style={{ minHeight: 64 }} value={bloc.texte} onChange={e => updateBloc(i, { texte: e.target.value })} />
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <div className="bac-form-row">
                      ...
                      <div className="bac-form-group" style={{ margin: 0 }}>
                        ...
                        <input className="bac-input" value={bloc.didascalie || ''} onChange={e => updateBloc(i, { didascalie: e.target.value })} />
                      </div>
                    </div>
                    <div className="bac-form-group" style={{ margin: 0 }}>
                      <label className="bac-label">Directive de jeu</label>
                      <input className="bac-input" value={bloc.directive} onChange={e => updateBloc(i, { directive: e.target.value })} />
                    </div>
                    <div className="bac-form-group" style={{ margin: 0 }}>
                      <label className="bac-label">Exemple de réplique</label>
                      <textarea className="bac-input bac-textarea" style={{ minHeight: 56 }} value={bloc.exemple} onChange={e => updateBloc(i, { exemple: e.target.value })} />
                    </div>
                  </div>
                )}
              </div>
      ...
      {tab === 'itw' && (
        <div>
          ...
          {form.itw_json.length === 0 && <div className="bac-empty"><p>Aucune question d'interview</p></div>}
              ...
              <div key={i} className="bac-card" style={{ padding: 16 }}>
                ...
                <div className="bac-form-row">
                  ...
                  <div className="bac-form-group" style={{ margin: 0, flex: 2 }}>
                    ...
                    <input className="bac-input" value={q.question} onChange={e => updateItw(i, { question: e.target.value })} />
                  </div>
                </div>
...
// ─── Page liste ───────────────────────────────────────────────────────────────
export default function AdminDenouements() {
  const [items, setItems] = useState<BacDenouement[]>([]);
  const [roles, setRoles] = useState<BacRole[]>([]);
  const [groupes, setGroupes] = useState<BacProfil[]>([]);
  const [loading, setLoading] = useState(true);
  const [editItem, setEditItem] = useState<BacDenouement | null>(null);
  ...
  async function createItem() {
    const res = await fetch('/bac/api/denouements', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ titre: 'Nouveau dénouement' }),
    });
    if (res.ok) {
  ...
  return (
    <div>
      <div className="bac-page-header bac-animate-in">
        <div>
          <h1>Dénouements</h1>
          <p style={{ color: 'var(--bac-text-secondary)', marginTop: 4 }}>Scènes de finale scripted</p>
        </div>
        <button className="bac-btn bac-btn-primary" onClick={createItem}>+ Nouveau dénouement</button>
      </div>
      ...
      {loading ? (
        ...
        <div className="bac-empty bac-animate-in">
          ...
          <p>Aucun dénouement créé</p>
          <button className="bac-btn bac-btn-primary" style={{ marginTop: 16 }} onClick={createItem}>Créer le premier dénouement</button>
        </div>
      ) : (
        <div className="bac-grid bac-grid-2 bac-stagger">
          {items.map(item => (
            <div key={item.id} className="bac-card bac-card-interactive"
              style={{ opacity: item.actif ? 1 : 0.55, cursor: 'pointer', borderLeft: '4px solid #f97316' }}
              onClick={() => setEditItem(item)}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                <h3 style={{ fontSize: '1.0625rem', fontWeight: 700 }}>🎬 {item.titre}</h3>
                <span className={`bac-badge ${item.actif ? 'bac-badge-success' : 'bac-badge-error'}`}>{item.actif ? 'Actif' : 'Inactif'}</span>
              </div>
