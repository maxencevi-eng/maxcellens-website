...
export function ImageCardsEditor({ data, onChange, lateral = false, pageOptions = [] }: Props) {
  ...
  return <>
    ...
    <AdminSection title="Cartes" description="Ajoutez vos images et leurs liens, puis choisissez leur ordre.">
      {data.cards.map((card, index) => <div key={index} style={{ border: '1px solid var(--admin-border, #ddd)', padding: 16, borderRadius: 8, display: 'grid', gap: 12 }}>
        ...
        <ImageField label="Image" value={card.image} onChange={image => updateCard(index, { image })} folder="pages/image-cards" page="page" deleteFromStorage={false} />
        ...
        <TextField label="Lien de la carte" value={card.href} onChange={href => updateCard(index, { href })} placeholder="/contact ou https://…" />
    ...
    <AdminSection title="Apparence et disposition" columns={2}>
